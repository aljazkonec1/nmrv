...
def get_patch(img, region):
    x0, y0, x1, y1 = region
    # padding
    if x0 >= img.shape[1] or y0 >= img.shape[0] or x1 < 0 or y1 < 0:
        return np.zeros((y1 - y0, x1 - x0, 3), dtype=np.float32), np.zeros((y1 - y0, x1 - x0), dtype=np.float32)
